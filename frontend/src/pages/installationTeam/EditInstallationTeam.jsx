    ...
    <div className="relative min-h-screen bg-deep-navy text-white overflow-hidden font-inter">
      ...
      <div className="relative z-10 px-6 md:px-12 mx-auto pb-20 pt-8">
        <div className="flex items-center justify-between mb-8">
          <Button variant="ghost" onClick={() => navigate(`/installation-teams/${id}`)} className="flex items-center gap-3 text-white/60 hover:text-white p-0 h-auto hover:bg-transparent">
          ...
          <h1 className="text-2xl font-black uppercase rim-light tracking-tighter">
            Edit <span className="text-solar-yellow">Installation Team</span>
          </h1>
        </div>
            ...
            <nav className="flex items-center gap-2 min-w-max border-b border-white/10">
              {TABS.map((tab) => (
                ...
                >
                  ...
                  {activeTab === tab.id && <motion.div layoutId="activeTabEditInst" className="absolute bottom-0 left-0 right-0 h-0.5 bg-solar-yellow shadow-[0_0_10px_rgba(255,215,0,0.5)]" />}
                </button>
              ))}
            </nav>
          </div>
                ...
                <div className="flex gap-4">
                  ...
                  {activeTab !== 'members' ? (
                    ...
                    <Button type="submit" disabled={loading} className="bg-white text-deep-navy font-black px-12 rounded-full hover:bg-solar-yellow hover:scale-105 transition-all shadow-[0_0_30px_rgba(255,255,255,0.2)]">
                      {loading ? 'COMMITTING...' : 'UPDATE TEAM'}
                    </Button>
                  )}
                </div>
              </div>
...
export default EditInstallationTeam;
