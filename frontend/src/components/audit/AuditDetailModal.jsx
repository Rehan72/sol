    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4">
          ...
          >
            {/* Header */}
            <div className={`p-6 border-b border-white/10 flex justify-between items-start ${log.phase === 'INSTALLATION' ? 'bg-solar-yellow/5' :
                log.phase === 'SURVEY' ? 'bg-emerald-500/5' : 'bg-white/5'
              }`}>
              <div>
                <div className="flex items-center gap-3 mb-2">
                  <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest border ${log.phase === 'INSTALLATION' ? 'border-solar-yellow text-solar-yellow' :
